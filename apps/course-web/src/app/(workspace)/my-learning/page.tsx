...
export default async function MyLearningPage({ searchParams }: { searchParams?: Promise<{ claim?: string }> }) {
  ...
  const practiceModule = next?.module ?? course.modules.find((module) => module.lectures.length) ?? firstModule;
  const nextHref = next
    ? `/courses/${courseSlug}/modules/${next.module.slug}${next.lecture.anchor ? `#${next.lecture.anchor}` : ""}`
